import VERT_SRC from '$lib/shaders/basic.vert?raw';
...
export class WebGL2Renderer {
	...
	constructor(
		...
		this.inPositionLocation = this.gl.getAttribLocation(this.program, POSITION_ATTRIBUTE_NAME);
		this.uTimeLocation = this.gl.getUniformLocation(this.program, TIME_UNIFORM_NAME);
		this.uResolutionLocation = this.gl.getUniformLocation(this.program, RESOLUTION_UNIFORM_NAME);

		this.vertexBuffer = createSimpleWebGL2VertexBuffer(this.gl, createQuadStrip2D());
...
function createWebGL2Program(gl: WebGL2RenderingContext, fragSrc: string): WebGLProgram {
	...
	try {
		...
		const success = ensureGlBoolean(gl.getProgramParameter(program, gl.LINK_STATUS));
		if (!success) {
...
function createWebGL2Shaders(gl: WebGL2RenderingContext, fragSrc: string): WebGLShader[] {
	const shaders = [];
	try {
...
function createWebGL2Shader(gl: WebGL2RenderingContext, type: number, src: string): WebGLShader {
	const shader = gl.createShader(type)!;
	try {
		gl.shaderSource(shader, src);
		gl.compileShader(shader);
		const success = ensureGlBoolean(gl.getShaderParameter(shader, gl.COMPILE_STATUS));
		if (!success) {
